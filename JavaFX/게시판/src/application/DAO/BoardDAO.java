...
public class BoardDAO extends JDBConnection {
	...
	public List<Board> list(){
		// 데이터 목록을 담을 List 객체 생성
		List<Board> boardList = new ArrayList<Board>();
		// SQL 작성
		String sql = " SELECT * " 		// 띄어쓰기 하기
				   + " FROM board ";
		// 데이터 처리 작업
		try {
			...
			System.err.println("데이터 목록 조회 시, 예외 발생");
		}
		// 4. 데이터 목록 반환
		return boardList;
	}
	...
	public Board select(int no) {
		...
		// 데이터 처리 작업
		try {
			// 1. SQL 실행 객체 생성 - prepareStatement (psmt)
			...
			System.err.println("데이터 조회 시, 예외 발생");
			e.printStackTrace();
		}
		// 4. 데이터 1건 반환
		return board;
	...
	public int inser(Board board) {
		...
		try {
			...
			System.err.println("데이터 등록 시, 예외 발생");
			e.printStackTrace();
		}
		return result;
	}
	...
	public int update(Board board) {
		...
		try {
			...
			psmt.setString(2, board.getWriter());	// 2번 ? 에 writer(작성자) 세팅
			psmt.setString(3, board.getContent());	// 3번 ? 에 content(내용) 세팅
			psmt.setInt(4, board.getNo());			// 4번 ? 에 no(글번호) 세팅
			result = psmt.executeUpdate();			// SQL 실행 요청
			// * executeUpdate()
			// SQL(INSERT, UPDATE, DELEDTE) 실행 시 적용된 데이터 개수를 int 타입으로 반환한다.
			// ex) 게시글 1개 적용 성공 시, result : 1
			...
			System.err.println("데이터 수정 시, 예외 발생");
			e.printStackTrace();
		}
		return result;
	}
	...
	public int delete(int no) {
		...
		try {
			psmt = con.prepareStatement(sql);		// 쿼리 실행 객체 생성
			psmt.setInt(1, no);						// 1번 ? 에 no(글번호) 세팅
			result = psmt.executeUpdate();			// SQL 실행 요청
			// * executeUpdate()
			// SQL(INSERT, UPDATE, DELEDTE) 실행 시 적용된 데이터 개수를 int 타입으로 반환한다.
			// ex) 게시글 1개 적용 성공 시, result : 1
			...
			System.err.println("데이터 삭제 시, 예외 발생");
			e.printStackTrace();
		}
		return result;
	}
